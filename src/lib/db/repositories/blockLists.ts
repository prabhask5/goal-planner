...
export async function getBlockLists(userId: string): Promise<BlockList[]> {
  const lists = await db.blockLists.where('user_id').equals(userId).toArray();
...
export async function createBlockList(
  ...
  const existingLists = await db.blockLists.where('user_id').equals(userId).toArray();
...
export async function deleteBlockList(id: string): Promise<void> {
  ...
  const websites = await db.blockedWebsites.where('block_list_id').equals(id).toArray();
