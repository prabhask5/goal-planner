...
import type { ConflictHistoryEntry } from '$lib/types';
...
export async function getPendingOpsForEntity(entityId: string): Promise<SyncOperationItem[]> {
  const allPending = await db.syncQueue.where('entityId').equals(entityId).toArray();
...
export async function getConflictHistory(
  ...
  const entries = await db.conflictHistory
    .where('entityId')
    .equals(entityId)
    .reverse()
    .limit(limit)
